...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    // ✅ Create new token FIRST before deleting old one
    ...
    console.log("New token created in DB:", !!newTokenRecord);

    // ✅ Only delete old token AFTER new one is confirmed saved
    await prisma.refreshToken.deleteMany({
      where: { token },
    });
    console.log("Old token deleted from DB");
